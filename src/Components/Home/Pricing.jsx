...
function Pricing() {
  return (
    <>
      <div className="bg-gradient-to-br from-slate-950 via-white to-slate-950 min-h-screen md:text-white pt-16 pb-20 px-4 sm:px-8 md:px-12 lg:px-24 xl:px-32">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-stretch justify-between space-y-8 md:space-y-0 md:space-x-8">
          {pricing.cards.map((card, index) => (
            <div
              key={index}
              className="w-full l:w-1/3 bg-white text-slate-900 p-6 rounded-bl-2xl rounded-tr-2xl shadow-lg hover:shadow-2xl hover:shadow-black transition-all ease-in-out duration-300 shadow-white flex-1 relative border-t-2 border-b-2 border-cyan-950 pb-24 flex flex-col justify-between"
            >
